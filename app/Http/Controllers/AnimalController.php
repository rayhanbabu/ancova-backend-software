...
{
    public function animal_view(Request $request){
        try{  
     ...
     public function store(Request $request){
       ...
       $validator=\Validator::make($request->all(),[    
           'animal_name'=>'required|unique:animals,animal_name',
     ...
     if($validator->fails()){
            ...
            $model->animal_name=$request->input('animal_name');
   ...
   public function animal_edit(Request $request) {
     $id = $request->id;
     $data = Animal::find($id);
     return response()->json([
         'status'=>200,  
   ...
   public function animal_update(Request $request ){
       ...
       $validator=\Validator::make($request->all(),[    
         ...
         'animal_name'=>'required|unique:animals,animal_name,'.$request->input('edit_id'),
         'image'=>'image|mimes:jpeg,png,jpg|max:400',
      ]);
   ...
   if($validator->fails()){
         ...
         $model=Animal::find($request->input('edit_id'));
     if($model){
        $model->animal_name=$request->input('animal_name');
        $model->animal_status=$request->input('animal_status');
  ...
  public function animal_delete(Request $request) { 
      ...
      //      ]);
      //   }else{
          $model=Animal::find($request->input('id'));
  ...
  public function fetch(Request $request){
      $dept_id = $request->header('dept_id');
      $data=Animal::where('dept_id',$dept_id)->orderBy('id','desc')->paginate(10);
      return view('admin.animal_data',compact('data'));
   }
  ...
  function fetch_data(Request $request)
  {
   if($request->ajax())
   {
         ...
         $sort_type = $request->get('sorttype'); 
           ...
           $data = Animal::where('dept_id',$dept_id)
              ->where(function($query) use ($search) {
                   $query->where('animal_name', 'like', '%'.$search.'%')
                     ->orWhere('animal_status', 'like', '%'.$search.'%');
                   })->orderBy($sort_by, $sort_type)->paginate($range);
                  return view('admin.animal_data', compact('data'))->render();
